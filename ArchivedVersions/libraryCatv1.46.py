...
# Bindings() in turn calls self.getSearchTerms() which checks if the input in the interface has had the enter key pressed and takes the input terms
# It then runs the actual program (mainprogram()) with those search terms which also returns the results. Those results are then sent to the text.input
# of the interface and displayed
...
class MainWindow:
    def __init__(self,master):
        ...
        # Master Window
        self.master = master
        self.master.title('Library Cat V. 1.46')
    ...
    def updateResults(self,results):
        #inserts items into text box of interface
        for item in results:
...
# This is the main program which does all of the work accessing the internet, getting raw data, and filtering that data. The mainProgram is sequestered in to
# two main sections:
...
def mainProgram(searchTerms):
    ...
    '''This is the main program which conducts all of the work. It access the internet, formulates the search URL, grabs raw data, filters the raw data, and
       composes the lists with all the desired results.
