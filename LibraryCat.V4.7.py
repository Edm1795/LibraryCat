...
class ItemPopup:
    ...
    def load_item(self, object):
        ...
        Label(data_frame, text=object.getTitle(), font=self.fontTuple, anchor="w", justify="left").grid(row=0, column=0, sticky="w", pady=2)

        Label(data_frame, text=object.getSubTitle(), font=self.fontTuple, anchor="w", justify="left").grid(row=1, column=0, sticky="w", pady=2)

        Label(data_frame, text=object.getAuthors(), font=self.fontTuple, anchor="w", justify="left").grid(row=2, column=0, sticky="w", pady=2)

        Label(data_frame, text='Description: ' + object.getDescription(), font=self.fontTuple, wraplength=wrapInPix, anchor="w", justify="left").grid(row=3, column=0, sticky="w", pady=2)

        Label(data_frame, text=object.getCallNumber(), font=self.fontTuple, anchor="w", justify="left").grid(row=4, column=0, sticky="w", pady=2)

        Label(data_frame, text=object.getPubDate(), font=self.fontTuple, anchor="w", justify="left").grid(row=5, column=0, sticky="w", pady=2)
...
class MainWindow:
    def __init__(self, master):
        ...
        self.master = master
        self.master.title('Library Cat V. 4.7')
        ...
        for instance in results:  # results is a list of LibItem objects
            for format in listOfFormats:
                if instance.getFormat() == format[0]:
                    ...
                    if instance.getImage() == None:
                        label = Label(format[2], text=posting, anchor='w', justify='left',
                                      font=self.fontTuple, wraplength=wrapInPix,fg="white", bg='#112233')
                    else:
                        label = Label(format[2], text=posting, anchor='w', justify='left',
                                      compound="left", image=instance.getImage(), pady=5, padx=5,
                                      font=self.fontTuple, wraplength=wrapInPix,fg="white", bg='#112233')

                    self.gridLabel(self.gridCacheValList,label)
