...
class MainWindow:
    def __init__(self, master):
        ...
        self.master = master
        self.master.title('Library Cat V. 4.00')

        #initialize the grid value for the items in the display frames (-1 used so that first grid is actually 0)
        self.gridCacheValFrame1=-1
    ...
    def updateGridCacheValues(self,newValue):

        self.gridCacheValFrame1=newValue

    def gridLabel(self,gridCacheVal,label):

        gridCacheVal = gridCacheVal + 1

        label.grid(row=gridCacheVal, column=0, sticky="nsew", padx=10, pady=10)

        self.updateGridCacheValues(gridCacheVal)
        ...
        for instance in results:
            if instance.getFormat() == 'BK':
                ...
                if instance.getImage() == 0:
                    label = Label(self.frame1.scrollable_frame, text=posting, anchor='w', justify='left',
                                  font=self.fontTuple, fg="white", bg='#112233')
                else:
                    label = Label(self.frame1.scrollable_frame, text=posting, anchor='w', justify='left',
                                  compound="left", image=instance.getImage(), pady=10, padx=5,
                                  font=self.fontTuple, fg="white", bg='#112233')

                self.gridLabel(self.gridCacheValFrame1,label)
