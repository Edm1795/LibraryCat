...
# 54. Ver. 3.6 (if this doesn't work revert to ver. 3.52..; Interface now fills the screen, state('zoomed'); the columns also fill the screen, rowconfigure weighted correctly
...
class LibItem:
    ...
    def saveImage(self):
        ...
        try:  # if the item has no link connected with it, the urllib function will throw an error, therefore handle the error as a pass
            ...
            # Convert the image to a format that Tkinter can display and save to variable
...
class MainWindow:
    def __init__(self, master):
        ...
        # Master Window
        self.master = master
        self.master.title('Library Cat V. 3.6')

        # Maximize the window (but do not hide the taskbar)
        ...
        for instance in results:
            if instance.getFormat() == 'BK':
                ...
                if instance.getDescription() != '':  # if the description is None then do not print the description or the '\n'. (If Desc. was None it will already have been changed to a ''
                    ...
                    posting = posting + 'Description: ' + slicedText + '...' + '\n'
                ...
                if instance.getImage()==0: # if image is set to 0 (no image saved) do not load the label with an image
                    label = Label(self.labelFrame, text=posting, anchor='w', justify='left', font=(self.fontTuple[0], self.fontTuple[1]), fg="white", bg='#112233')
                    print('0 was found for iamge')
                else: # if the image variable contains an image, load label with image
                    label = Label(self.labelFrame, text=posting, anchor='w', justify='left', compound="left", image=instance.getImage(), pady=10, padx=5, font=(self.fontTuple[0],self.fontTuple[1]), fg="white",bg='#112233')
                label.pack(side='top', fill='x') # side=top packs labels vertically; fill=x fills unused space.
                self.canvas1.create_window((0, 0), window=self.labelFrame, anchor="nw")
                self.canvas1.config(scrollregion=self.canvas1.bbox("all"))  # Set the scrollable region
