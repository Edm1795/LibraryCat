...
class LibItem:
    def __init__(self,callNumber, authors, jacket, audiences, compositeSubjectHeadings, itemFormat, title, subtitle, primaryLanguage, publicationDate, description, subjectHeadings, materialType, totalCopies):
        '''
    ...
    def getImageLink(self, imageSize):
        ...
        '''
        if imageSize == 's':
            return jacket.get('small', '')
        elif imageSize == 'm':
            return jacket.get('medium', '')
        elif imageSize == 'l':
            return jacket.get('large', '')
        else:
            return ''
...
class MainWindow:
    def __init__(self, master):
        ...
        self.master = master
        self.master.title('Library Cat V. 4.4')
    ...
    def getSearchTerms(self):
        ...
        self.results = programFlow(searchTerms, self.getNumOfResultPages())
            ...
            for format in listOfFormats:
                if instance.getFormat() == format[0]:
                    ...
                    if instance.getDescription() != '':
                        posting += 'Description: ' + characterLimit(instance.getDescription(), 50) + '...\n'

                    posting += instance.getPubDate() + '\n\n'

                    if instance.getImage() == 0:
                        label = Label(format[2], text=posting, anchor='w', justify='left',
                                      font=self.fontTuple, fg="white", bg='#112233')
                    else:
                        label = Label(format[2], text=posting, anchor='w', justify='left',
...
def programFlow(searchTerms, numOfResultPages):
    ...
    while c < numOfResultPages: # run the loop according to the number of page results desired (determined by user on interface)
        ...
        for bib in dataDict['entities']['bibs'].values(): # enter the bib inside entities
            ...
            listOfItemObjects.append(LibItem(callNumber, authors, jacket, audiences, compositeSubjectHeadings, itemFormat, title, subtitle, primaryLanguage, publicationDate, description, subjectHeadings, materialType, totalCopies))
