...
# note: remember to handle no results for a given search
...
# 12. do a general fix of cases where a new format is found (ex. book_cd) so that any new format not listed in the storeData function will still maintain proper order of results.
...
def storeData(form,author,title,subtitle,description):
        ...
        if author != 1 and title != 1 and subtitle != 1 and description != 1 and (form == 'VIDEO_ONLINE' or form == 'BK' or form == 'GRAPHIC_NOVEL' or form == 'EBOOK' or form == 'MUSIC_ONLINE' or form == 'MUSIC_CD' or form == 'BLURAY' or form == 'DVD' or form == 'DIGITAL_SCORE' or form == 'AB_ONLINE' or form == 'BOOK_CD') :
                return [form,author,title,subtitle]

        elif form != 1 and (form != 'VIDEO_ONLINE' or form != 'BK' or form != 'GRAPHIC_NOVEL' or form != 'EBOOK' or form != 'MUSIC_ONLINE' or form != 'MUSIC_CD' or form != 'BLURAY' or form != 'DVD' or form != 'DIGITAL_SCORE' or form != 'AB_ONLINE' or form != 'BOOK_CD') and author != 1 and title != 1 and subtitle != 1 and description != 1:
                return [form, author, title, subtitle]  # This additional clause handles cases when an unknown format is found(something other than all those listed in the 'if' clause). In such a case, if all fields are full (including an unknown format) a list will still be returned with those items. It will not neccessarily get printed off however (depending on the settings in the print function. This should ensure a proper correspondance between data fields even when unknown formats exist in the raw data
