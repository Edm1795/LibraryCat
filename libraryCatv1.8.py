...
class MainWindow:
    def __init__(self, master):
        ...
        # Master Window
        self.master = master
        self.master.title('Library Cat V. 1.8')
        self.master.geometry("+200+300")  # position of window in the screen
        ...
        # Set up text boxes
        self.text1 = Text(self.frame1, height = 40, width = 100, bg='#112233', fg='white')
        ...
        self.text2 = Text(self.frame2, height = 40, width = 100, bg='#112233', fg='white')
        ...
        self.text3 = Text(self.frame3, height = 40, width = 100, bg='#112233', fg='white')
        ...
        self.text4 = Text(self.frame4, height = 40, width = 100, bg='#112233', fg='white')
    ...
    def getSearchTerms(self):
        ...
        self.results = programFlow(searchTerms, 3) # 1. send searchTerms; 2. specify number of pages of results to get from mainsite; note can not go over 4 pages
    ...
    def updateResults(self, results):
        ...
        for tup in results:  # each tup is one page of results
            for list1 in tup:  # [[]] list of lists where each list is one item type
                for item in list1:  # each inidividual item of one type []  This is the key part of the data structure; it is like one book or one DVD.
                    if item[0] == 'BK':  # index to each element of a single item
                        for element in item:  # zero in on each element of a library item
                            if element == None:  # Check if popped item is None
                                element = ' '  # if None, replace with empty string (so as to avoid tkinter error: .insert() can not accept None values)
                            self.text1.insert('end', element)  # inserts each element into the interface
                            self.text1.insert('end', ' ')  # insert space between element
                        self.text1.insert('end', ' \n\n')  # puts empty line between each library item
                    elif item[0] == 'DVD':
                        for element in item:  # zero in on each element of a library item
                            if element == None:  # Check if popped item is None
                                element = ' '  # if None, replace with empty string (so as to avoid tkinter error: .insert() can not accept None values)
                            self.text2.insert('end', element)  # inserts each element into the interface
                        ...
                        for element in item:  # zero in on each element of a library item
                            if element == None:  # Check if popped item is None
                                element = ' '  # if None, replace with empty string (so as to avoid tkinter error: .insert() can not accept None values)
                            self.text3.insert('end', element)  # inserts each element into the interface
                        ...
                        for element in item:  # zero in on each element of a library item
                            if element == None:  # Check if popped item is None
                                element = ' '  # if None, replace with empty string (so as to avoid tkinter error: .insert() can not accept None values)
                            self.text4.insert('end', element)  # inserts each element into the interface
