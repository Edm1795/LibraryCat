...
class MainWindow:
    def __init__(self, master):
        ...
        # Master Window
        self.master = master
        self.master.title('Library Cat V. 2.1')
        ...
        self.bindings(
            master)  # the bindings() function initiates the whole program flow by checking if the return key has been depressed in the interface
    ...
    def getSearchTerms(self):
        ...
        numOfResultPages = 3 #  Number of result pages desired from main source
        ...
        if self.count == 1:  # If first time run of search (of program itself from search bar), no need to clear text fields
            self.count += 1
        ...
        for instance in results:
            if instance.getFormat() == 'BK':
                ...
                if instance.getDescription() != '':  # if the description is None then do not print the description or the '\n'. (If Desc. was None it will already have been changed to a ''
                    self.text1.insert('end', 'Description: ' + instance.getDescription()+'\n')
